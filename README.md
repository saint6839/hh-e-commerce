...
서비스의 코드를 깔끔하고 유지보수에 용이하게 작성하는것도 중요하지만, 더 중요한 것은 서비스의 성능이다. 서비스의 성능은 사용자의 경험을 좌우하기 때문에, 서비스의 성능을 테스트하고 최적화하는 것은 매우 중요하다.

현재까지 이 프로젝트를 진행해오며, 다양한 상황에서의 성능 최적화와 동시성 제어에 대한 테스트를 진행하였다. 이렇게 우리가 만든 api들이 실제 수 많은 사용자들이 사용하는 상황에서도 충분히 빠르고 정확하게 응답할 수 있는지를 확인하고, 문제점을 찾아 개선하기 위해 부하테스트를 진행하였다.

## 부하테스트 시나리오와 테스트 결과

api들 중에서 주요 부하 테스트 대상들을 선정하였다. 그리고 각 대상들에 대한 테스트 시나리오는 다음과 같다. 몇번의 로컬에서 부하 테스트를 시험해본 결과 로컬 기기의 사양의 한계로 인해 충분히 부하를 주지 못하는 경우가 많았다. 이를 고려하여 성공 기준을 설정하게 되었다.
...
- 모든 API의 평균 응답 시간이 2초 미만 유지
- 오류율 1% 미만
- CPU 사용률 80% 미만
- 초당 처리 요청 수 (TPS) 목표: 최소 20 TPS

### 1. 사용자 포인트 충전 API

- 목적 : 동시에 많은 사용자가 포인트를 충전하거나 사용할 때 시스템의 안정성과 정확성 검증
- 단계
  - 포인트 충전 테스트
    - 초기 30초 동안 초당 20명의 가상 사용자로 시작 (총 600명)
    - 다음 30초 동안 초당 50명의 가상 사용자로 증가 (총 1500명)
    - 마지막 30초 동안 초당 100명의 가상 사용자로 최대 부하 테스트 (총 3000명)
    - 각 사용자가 랜덤한 금액의 포인트를 충전 (금액 범위는 스크립트에 정의됨)
    - 각 요청 사이에 1초의 대기 시간 설정

#### 결과

- CPU, 메모리 사용량

  ![alt text](charge-point-cpu.png)

- [부하테스트 결과](artillery/point-charge/point-charge-report.json)

1. 전체 요약:

   - 총 5,100개의 가상 사용자 생성
   - 모든 요청 성공적으로 완료 (실패 없음)
   - HTTP 요청 비율: 초당 60개
...
3. 세션 길이:
   ...
   - 최소: 1001.3ms, 최대: 1109.2ms
   - 평균: 1005.4ms
   - 대부분의 세션(90%)이 1002.4ms 이내에 완료

4. 단계별 분석:
   - 초기 단계(20 TPS): 평균 응답 시간 3.5ms
   - 중간 단계(50 TPS): 평균 응답 시간 2.4ms
   - 최종 단계(100 TPS): 평균 응답 시간 3.7ms

위 테스트 결과와 CPU, 메모리 사용량에서 알 수 있듯이 사용자 포인트 충전 로직에 대한 부하 테스트는 성공적이었다. 그렇다면 어떤 이유로 낮은 로컬 기기의 사양과 많은 부하에도 불구하고 사용자 충전 로직은 성공적인 결과를 반환할 수 있었을까?

먼저, 로직 자체의 연산이 단순하다는 점을 꼽을 수 있다. 로직 자체에서 단순한 덧셈 로직만 포함되어있기 때문에, 복잡한 계산이나 처리가 필요하지 않기 때문에, 앞선 요청들이 빠르게 처리될 수 있었고 많은 요청을 처리하는데 도움을 주었다. 또한 각 사용자의 잔액 충전은 서로 독립적인 트랜잭션으로 처리되기 때문에, 동시에 여러 사용자의 충전 요청이 들어오더라도 서로 간섭하지 않기 때문에 더욱 빠르게 처리될 수 있었다.

### 2. 상품 목록 조회 API

- 목적 : 대량의 동시 접속자가 상품 목록을 조회할 때 시스템 성능 확인
- 단계
  1. 10초 동안 초당 10명의 사용자가 요청 (총 100명)
  2. 다음 10초 동안 초당 20명의 사용자로 증가 (총 200명)
  3. 다음 10초 동안 초당 50명의 사용자로 증가 (총 500명)
  4. 마지막 10초 동안 초당 100명의 사용자로 최대 부하 테스트 (총 1000명)
- 각 사용자는 상품 목록을 조회한 후 10초간 대기

- CPU, 메모리 사용량

  ![alt text](product-list-cpu.png)

- [부하테스트 결과](artillery/product-list/product-list-report.json)
...
3. 세션 길이:
   ...
   - 최소: 1,009.4ms, 최대: 2,723.9ms
   - 평균: 1,051.8ms
   - 대부분의 세션(95%)이 1,200.1ms 이내에 완료

4. 단계별 분석:
   - 초기 단계 (10 TPS): 평균 응답 시간 362ms, 최대 1,720ms
   - 중간 단계 (20 TPS): 평균 응답 시간 12.7ms, 최대 68ms
   - 중간 단계 (50 TPS): 평균 응답 시간 14.4ms, 최대 125ms
   - 최종 단계 (100 TPS): 평균 응답 시간 73.9ms, 최대 330ms

위 테스트가 성공적으로 마무리 될 수 있었던 가장 큰 이유는 캐싱 로직이 적용되어있기 때문이다.

결과에서 확인할 수 있었듯 캐싱되어있지 않은 첫 요청에서는 느린 응답을 보이기도 했었지만,
이후부터는 캐싱된 데이터를 빠르게 반환할 수 있었고, 이는 더욱 많은 요청을 처리하는데 도움을 주었다. 만약, 캐싱 로직이 없었더라면 각 상품별 옵션을 조회해오는 과정에서 많은 시간이 소요되었을 것이다. 이는 각 요청의 처리 시간이 더뎌지는것을 의미한다.

실제로 이 테스트에서 캐싱과 인덱스를 제외하고 부하 테스트를 진행하였을 때, 초반의 요청에 대해서는 어느정도 처리하는 모습을 보였으나 점점 요청 횟수가 증가하는 후반부에서는 서버가 요청을 소화하지 못하고 대부분의 요청이 timeout으로 실패하는 것을 확인하였다.

### 3. 인기 상품 목록 조회 API

- 목적 : 특정 시간대에 인기 상품 목록 조회가 집중될 때 시스템 반응 확인
- 단계:
  - 초기 30초 동안 가상 사용자를 10명에서 100명으로 급격히 증가
    - 초당 사용자 수가 10명에서 100명으로 선형적으로 증가
  - 이후 30초 동안 100명의 가상 사용자 수 유지
  - 각 사용자가 3초 간격으로 인기 상품 목록 조회 API를 호출
...
3. 세션 길이:
   ...
   - 최소: 1,000.9ms, 최대: 1,193.6ms
   - 평균: 1,004.8ms
   - 대부분의 세션(95%)이 1,022.7ms 이내에 완료
...
마찬가지로 인기 상품 조회로직도 부하테스트를 잘 견뎌내는 모습을 확인할 수 있었다. 이 또한 상품 조회 api와 마찬가지로 적절하게 캐싱과 인덱싱이 되어있었기 때문이다.

### 4. 결제 확정 API

- 목적: 동시 다발적인 결제 요청 처리 시 시스템 안정성 검증
- 단계:
  - 초기 30초 동안 초당 20명의 가상 사용자로 시작
  - 다음 30초 동안 초당 50명의 가상 사용자로 증가
  - 마지막 30초 동안 초당 100명의 가상 사용자로 최대 부하 테스트
  - 각 사용자가 1초 간격으로 결제 API 호출
- CPU, 메모리 사용량

  ![alt text](payment-cpu.png)

- [부하테스트 결과](artillery/payment-confirmation/payment-confirmation-report.json)

1. 전체 요약:

   - 총 5,100개의 가상 사용자 생성
   - 성공적으로 완료된 요청: 2,167
   - 실패한 요청: 2,933 (ETIMEDOUT 오류)
   - HTTP 요청 비율: 초당 47개
...
3. 단계별 분석:
   - 초기 단계(20 TPS): 안정적인 성능, 모든 요청 성공
   - 중간 단계(50 TPS): 성능 유지, 대부분의 요청 성공
   - 최종 단계(100 TPS): 성능 저하, 대부분의 요청 시간 초과

이전의 단순 조회로직에서는 100 TPS까지 대부분 잘 처리되는 모습을 보였으나, 결제 확정 로직에서는 50 TPS 이상으로 부하가 늘어나게되면 처리가 느려지는 모습을 보였다. 이를 추가적으로 더 개선하기 위해서는 현재도 어느정도 이벤트로 분리되어있기는 하지만, 이벤트 기반 아키텍처를 통해 핵심 비즈니스 로직과 부가적인 작업을 분리하는 것이 필요할 것으로 판단된다. 이를 통해 각 요청의 트랜잭션이 유지되는 시간을 짧게 가져가도록 하여, 많은 부하를 처리하는데 도움을 줄 수 있다.

### 사용자 충전 -> 주문 -> 결제 전체 시나리오 테스트

- 목적: 사용자 포인트 충전, 주문 생성, 결제 확정을 포함한 전체 주문 프로세스의 시스템 안정성 및 성능 검증
- 단계:
  - 초기 30초 동안 초당 10명의 가상 사용자로 시작하여 30명까지 점진적 증가
  - 다음 60초 동안 초당 30명의 가상 사용자로 지속적인 부하 유지
  - 각 사용자가 다음 순서로 API를 호출:
    1. 사용자 포인트 충전 API 호출
    2. 1초 대기
    3. 주문 생성 API 호출
    4. 1초 대기
...
1. 전체 요약:
   ...
   - 총 2,400개의 가상 사용자 생성
   - 총 7,200개의 API 요청 (충전, 주문, 결제 각 2,400개)
   - 성공적으로 완료된 요청: 7,182개
   - 실패한 요청: 18개 (모두 주문 생성 API에서 발생)
   - HTTP 요청 비율: 초당 평균 69개 (최대 90개)
...
사용자의 잔액 충전부터 주문과 결제 확정까지 전체 흐름을 하나의 시나리오로 묶어 부하테스트를 진행해보았다.

대부분의 api가 매우 빠른 응답시간을 보여주고 있으며, 특히 결제 확정 api의 경우 최대 157ms의 응답시간을 보여주고 있음에도 불구하고 전체 성공률은 99.75%로 매우 높은 수준을 보여주고 있음을 알 수 있었다.

전체적으로 리뷰해보았을때, DB 자체의 부하가 심해져서 발생하는 문제는 확인할 수 없었다. 이는 대부분의 쿼리나 트랜잭션이 잘 최적화 되어있는 것으로 생각해볼 수 있을 것 같다. 그러나 더 많은 사용자와

다만, 로컬 기기 메모리의 한계로 더 큰 부하를 줄 경우, 프로세스들이 강제로 종료되는 문제를 맞닥뜨렸다. 실제 운영환경에서 이 테스트 시나리오에서 진행한 조건 이상의 부하가 예상되는 경우에는 현재 스펙 이상의 서버에 배포를 진행하거나, 스케일 아웃을 통해 부하를 분산해줄 필요가 있어보인다.

### Cold Cache Issue

부하테스트를 진행하다보니 한 가지 우려되는 점이 있었다. 분명 캐싱 기능이 존재하는 로직에 요청을 보냈는데, 최대 응답 시간이 캐싱된 시간보다 더 오래 걸리는 경우가 있었다. 이는 캐싱된 데이터가 캐시에 존재하지 않아서 캐싱을 하는 과정에서 응답 시간이 길어진 것이었다.

이것은 사용자들이 빠른 응답을 받기 위해서는 우리 서비스를 이용하는 사용자들 중 누군가가 캐싱되어 있지 않는 상태를 겪어야만 한다는것을 의미했다. 이러한 부분에 대해서 Cold Cache Issue라는 용어로 표현하고 있다.

이 프로젝트에서는 모든 사용자가 특정 요청에 대해서 캐싱된 데이터를 조회함으로써 빠른 응답을 받기를 기대하였기 때문에, 이 문제를 해결하고자 스케줄러를 통해 사용자가 아닌 서버에서 주기적으로 새로운 캐싱을 해줌으로써 모든 사용자에게 빠른 응답이 보장될 수 있도록 개선하였다.

이 결과를 테스트 하기 위해서 우리 서비스 중에서 많은 양의 데이터를 조회하고, 캐싱이 되어있는 API인 상품 조회와 관련된 API에 대해서 부하테스트를 진행하였고, 부하테스트 도중 캐싱된 데이터가 expiration 되어 사라지는 상황을 재현하였다.

![alt text](cold-cache.png)

- 빨간색 : 캐싱 전
- 파란색 : 캐싱 후 & cold cache issue 발생
- 초록색 : 캐싱 후 & cold cache issue 해결

빨간 그래프에서 볼 수 있듯이 캐싱 전에는 전체적으로 모든 요청에 대해서, 상대적으로 느린 응답 시간을 일관적으로 보인다.

두 번째 파란색 그래프에서는 캐싱된 데이터를 조회하기 때문에, 빠른 응답시간을 보이지만 캐싱된 데이터가 없는 경우 캐싱을 하는 과정에서 응답 시간이 길어지는 것을 확인할 수 있다.

세 번째 초록색 그래프에서는 언제나 캐싱된 데이터를 조회하기 때문에, 모든 요청에 대해서 일관되게 빠른 응답을 보이는 것을 확인할 수 있었다.
